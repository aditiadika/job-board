                ...
                @foreach ($tags as $tag)
                    <a href="{{ route('listings.index', ['tag' => $tag->slug]) }}"
                       class="inline-block ml-2 tracking-wide text-xs font-medium title-font py-0.5 px-1.5 border border-indigo-500 uppercase
                              {{ $tag->slug == request('tag') ? 'bg-indigo-500 text-white' : 'bg-white text-indigo-500' }}"
                    >
                        {{ $tag->name }}
                    </a>
                @endforeach
            ...
            @foreach ($listings as $listing)
                ...
                >
                    <div class="md:w-16 md:mb-0 mb-6 mr-4 flex-shrink-0 flex flex-col">
                        <img src="https://dummyimage.com/300" alt="logo" class="w-16 h-16 rounded-full object-cover">
                    </div>

                    <div class="md:w-1/2 mr-8 flex flex-col item-start justify-center">
                    ...
                    <div class="md:flex-grow mr-8 flex items-center justify-start">
                        @foreach ($listing->tags as $tag)
                            <span class="inline-block ml-2 tracking-wide text-xs font-medium title-font py-0.5 px-1.5 border border-indigo-500 uppercase {{ $tag->slug == request('tag') ? 'bg-indigo-500 text-white' : 'bg-white text-indigo-500' }}">
                                {{ $tag->name }}
                            </span>
                        @endforeach
                    </div>
